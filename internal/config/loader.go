...
// LoadWithWarnings mirrors Load but returns missing env var names instead of printing.
func LoadWithWarnings(path string) (Config, []string, error) {
	...
	if err != nil {
		return Config{}, nil, apperr.Wrap("config.Load", apperr.InvalidInput, err, "abs path")
	}

	b, err := os.ReadFile(guessedAbs)
	if err != nil {
		return Config{}, nil, apperr.Wrap("config.Load", apperr.NotFound, err, "read config")
	}
	...
	if err := dec.Decode(&cfg); err != nil {
		return Config{}, missing, apperr.New("config.Load", apperr.InvalidInput, "parse yaml: %s", yaml.FormatError(err, true, true))
	}
...
// RenderWithWarnings returns interpolated YAML and missing env var names.
func RenderWithWarnings(path string) (string, []string, error) {
	...
	guessedAbs, err := filepath.Abs(guessed)
	if err != nil {
		return "", nil, apperr.Wrap("config.Render", apperr.InvalidInput, err, "abs path")
	}

	b, err := os.ReadFile(guessedAbs)
	if err != nil {
		return "", nil, apperr.Wrap("config.Render", apperr.NotFound, err, "read config")
	}
