...
func TestInvalidComposePlanAndApply(t *testing.T) {
	...
	if !strings.Contains(out, "invalid compose file") || !strings.Contains(out, "for stack app") {
	...
	if !strings.Contains(out2, "invalid compose file") || !strings.Contains(out2, "for stack app") {
		t.Fatalf("expected invalid compose error message, got:\n%s", out2)
	}
}
